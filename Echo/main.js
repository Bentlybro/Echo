...
const setupLogging = () => {
  try {
    ...
    const logFile = path.join(logsDir, `echo-${new Date().toISOString().split('T')[0]}.log`);
    ...
    console.log('Echo Music Player starting...');
...
process.on('uncaughtException', (error) => {
  ...
  dialog.showErrorBox('Fatal Error', 
    `Echo has encountered a fatal error and needs to close.\n\nError: ${error.message}\n\nCheck logs in: ${app.getPath('userData')}/logs/`
  );
});
